...
3. **Amélioration** — les propositions sont affinées à la lumière des critiques.
...
- **Bornes strictes** : le débat est limité dans le temps (time-box) et en nombre d'itérations. L'atteinte d'une borne déclenche une sortie explicite (recommandation ou escalade), jamais une boucle sans fin.
...
Le débat ne s'ouvre que si le **quorum** est atteint : un nombre minimal de membres présents et couvrant les expertises indispensables à la question.
...
La taille du conseil est bornée. Si la question est trop large pour un conseil de taille raisonnable, le facilitateur la découpe en sous-questions confiées à des **sous-comités**, dont les conclusions seront ensuite réunies.
...
Le facilitateur veille à l'équité du temps de parole et empêche qu'un membre monopolise le débat.

## Critique et amélioration

La critique n'est pas une fin : elle sert à améliorer les propositions.
...
Exemple de challenge → amélioration : une proposition « lancer sur le marché A » est critiquée pour un risque réglementaire ; l'auteur l'amende en « lancer sur le marché A avec une phase pilote limitée pour lever l'incertitude réglementaire ». La proposition affinée remplace l'ancienne.
...
- **Pas de vote couperet** : le conseil ne « vote » pas pour faire disparaître une position minoritaire. Le rôle du conseil est d'éclairer, pas de trancher à la place du CEO.
...
Le conseil est réputé avoir convergé lorsque **la liste des désaccords ouverts ne diminue plus d'un tour à l'autre** : un tour de critique complet s'achève sans qu'aucune position ne soit révisée ni aucun désaccord tranché. La stabilité, et non l'unanimité, est le signal d'arrêt.

### Time-box

Chaque débat est limité dans le temps. Le facilitateur annonce la time-box à l'ouverture et signale son approche. À échéance, le débat s'arrête même si la convergence n'est pas atteinte : on passe à la production de la recommandation en l'état.

### Nombre maximal d'itérations

Le nombre de tours de critique est plafonné. Conditions de sortie :

- **Si convergence atteinte avant la borne** : le débat s'arrête et passe à la recommandation.
- **Si la time-box ou le nombre maximal d'itérations est atteint sans convergence** : le débat s'arrête aussi, mais les options restées en désaccord sont présentées **à parité** (aucune n'est privilégiée) et la situation est **escaladée au CEO** pour arbitrage.
...
5. **Désaccords et positions minoritaires** : ce qui n'a pas fait l'unanimité, avec l'argumentaire de chaque camp et le nombre de soutiens.
6. **Éventuelle option privilégiée** : si — et seulement si — le conseil a convergé, l'option qu'il recommande, avec la justification de la préférence. En cas de non-convergence, cette rubrique est explicitement remplacée par une présentation des options à parité.
7. **Renvoi de décision** : mention rappelant que le choix final appartient au CEO.

La recommandation est transmise selon le protocole de décision (voir [`./05-decision-protocol.md`](./05-decision-protocol.md)).

## Exemple concret

Cas simple : un conseil de trois membres — un membre « marché », un membre « produit », un membre « risque » — doit répondre à « Faut-il facturer le nouveau service à l'usage ou par abonnement ? ». Quorum atteint, time-box fixée, maximum de deux tours de critique.
...
- **Tour 1 — Critique.**
  ...
  - Le membre produit répond et **amende** sa proposition : « à l'usage, avec un plafond mensuel optionnel » pour lisser le revenu. Version consignée.
...
- **Recommandation produite.** Deux options considérées (abonnement ; à l'usage avec plafond optionnel), leurs raisons et risques respectifs, une position minoritaire préservée (le membre marché maintient sa préférence pour l'abonnement, consignée), et une **option privilégiée** : « à l'usage avec plafond optionnel », soutenue par deux membres sur trois. Le tout est remonté au CEO, à qui revient la décision.

## Cas limites

- **Impasse persistante** — Si, à l'épuisement de la time-box ou du nombre maximal d'itérations, des désaccords majeurs subsistent : le conseil ne force pas une conclusion. Il présente les options restantes **à parité** et **escalade au CEO** pour arbitrage.
- **Égalité stricte de positions** — Si deux options recueillent un soutien identique et qu'aucun tour supplémentaire n'est autorisé : le conseil ne départage pas artificiellement. Les deux options sont présentées **à parité**, avec leurs argumentaires équivalents, et la décision est renvoyée au CEO.
- **Expertise manquante** — Si le débat révèle qu'une compétence indispensable n'est pas représentée : le facilitateur **signale la lacune**. Il tente de mobiliser le membre manquant (au besoin via un sous-comité) ; si c'est impossible dans le cadre imparti, il **escalade au CEO** en indiquant que la recommandation est produite sous réserve de cette lacune.
