...
| **T15 [O]** | Agnosticisme technologique | Même objectif dans deux contextes (A : construire est pertinent ; B : une solution existante est meilleure) | Deux recommandations de **nature différente**, justifiées par le contexte et comparées sur les **mêmes critères** | A → construire ; B → intégrer/acheter ; justification économique explicite ; la nature de la réponse découle du contexte, jamais d'une prémisse | Même réponse dans les deux cas ; le code (ou l'achat) posé en prémisse | Deux recommandations (14 champs) de nature différente, avec leur justification contextuelle — **aucun plan de capacités requis** (T17, V1.5) |
...
| **T15** | Une seule forme de sortie ; le pipeline produit toujours un « plan de solution » à construire | `agents.py:95-105` | Pas d'option « intégrer / acheter » ; la nature de la réponse ne dépend pas du contexte | Options du Tour 0 (T06) ; option nulle (T13) | Les options du Tour 0 admettent explicitement « construire / intégrer / acheter / ne rien faire » ; la synthèse justifie la nature retenue par le contexte ; **aucune dépendance à T17/T18** (V1.5) |
