...
- **Rapport de préparation** : audit de 12 composants ; bilan réutilisé / étendu sans rupture / non réutilisé-mais-conservé (**rien n'est supprimé**) ; incrément 1 « Cadrage + Exploration indépendante + Cartographie + Rapport de situation » (1 + N + 0–1 appels LLM, un appel isolé par expert, composition et cartographie déterministes, rapport `candidate` soumis au CEO) ; tests à améliorer ; 11 risques doctrinaux ; verdict.
...
- *Ne pas conserver le commit de handoff pré-existant* — rejeté au profit de la règle de non-perte ; signalé comme périmé et hors mandat pour décision CEO.

## 7. Risques

- Lecture de la clause 7 comme une autorisation d'exécuter (la décision dit explicitement le contraire).
- Réintroduction d'un quota d'experts par un futur incrément (garde-fou : tests « courant → ≤ 2 », expérience obligatoire).
- Le handoff périmé pris pour source de vérité (à retirer ou marquer).
